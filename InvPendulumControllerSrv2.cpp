...
public:
  ...
  double control(double _dq, const Vector3d &_omega, const Vector3d &_accel)
  {
    ...
    //std::cerr << u_torque << " / " << - rl_torque << std::endl;
    //u_torque = - rl_torque;
    /// torque limitation
...
class InvPendulumController : public SimpleController
{
  ...
  std::condition_variable act_cond_;
...
public:
  InvPendulumController() : advertised(false)
  {
  }
  ...
  bool service_cb (aizu::Control::Request &req,
                   aizu::Control::Response &res)
  {
    {
    std::lock_guard<std::mutex> lk(act_mtx_);
      if (req.state.size() >= 1) {
        action.push_back(req.state[0]);
        no_block = false;
      } else {
        action.push_back(0);
        no_block = true;
      ...
      //ROS_WARN("a=>");
      act_cond_.notify_all();
    }

    {
      std::unique_lock<std::mutex> slk(state_mtx_);
      state_cond_.wait(slk, [&] {
          if (state.size() <= 3) {
            return false;
          }
          return true;
      ...
      res.action.resize(4);
      res.action[0] = state[0];
      res.action[1] = state[1];
      res.action[2] = state[2];
      res.action[3] = state[3];
      state.resize(0);
    }
  ...
  virtual bool initialize(SimpleControllerIO* io) override
  {
    ...
    std::cout << "initialized[InvPendulumController] " << (void *)this << std::endl;

    /// service
    if (!advertised) {
      srv = nh->advertiseService("/chainerrl/control2", &InvPendulumController::service_cb, this);
      advertised = true;
    }

    counter = 0;
    //start_count = 200 * (rnd() / (double)std::random_device::max()) + 500;
  ...
  virtual bool control() override
  {
...

    {
      ...
      state.resize(0);
      state.push_back(theta_act);
      state.push_back(d_theta_act);
      state.push_back((wheel_l->q() + wheel_r->q())/2 + theta_act);
      state.push_back((wheel_l->dq() + wheel_r->dq())/2 + d_theta_act);
      //ROS_WARN("s=>");
      state_cond_.notify_all();
    }

    double action_torque = 0.0;
#if 0
    /// using actual value
    ...
    if (!no_block) {
      //ROS_WARN("a<-");
      std::unique_lock<std::mutex> alk(act_mtx_);
      act_cond_.wait(alk, [&] {
          if (action.size() == 0) {
            return false;
          }
    ...
    double u_torque = - impl->control(-dq + vx_ref, omega, accel);

    u_torque = action_torque;
...
/*
/usr/bin/c++ -DInvPendulumControllerSrv2_EXPORTS -I/home/leus/choreonoid_ws/devel/include -I/home/leus/choreonoid_ws/devel/include/choreonoid-1.7 -I/usr/include/eigen3 -I/opt/ros/kinetic/include -O3 -DNDEBUG -fPIC -std=c++11 -c InvPendulumControllerSrv2.cpp

/usr/bin/c++  -fPIC -O3 -DNDEBUG  -shared -Wl,-soname,InvPendulumControllerSrv2.so InvPendulumControllerSrv2.o -o InvPendulumControllerSrv2.so -lroscpp -L/opt/ros/kinetic/lib
*/
